...
Complete setup and usage guide for integrating the self-improvement skill with NEURO.

## Overview

NEURO uses workspace-based prompt injection combined with event-driven hooks. Context is injected from workspace files at session start, and hooks can trigger on lifecycle events.
...
### 1. Skill is Already Installed

The skill lives at `NEURO/skills/self-improvement/` — no additional steps needed.

### 2. Install the Hook (Optional)

Copy the hook to NEURO's hooks directory:
...
Or copy the starter files from the skill's assets:

```bash
cp skills/self-improvement/assets/LEARNINGS.md .learnings/LEARNINGS.md
```
...
NEURO provides tools for cross-session communication:
...
| Identity issue | Log to `IDENTITY.md` with persona details |
...
1. Check skill is in `NEURO/skills/` directory
2. Verify `SKILL.md` has correct frontmatter
3. Run `neuro status` to see loaded skills
