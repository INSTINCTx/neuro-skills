...
This injects the self-improvement reminder at every `agent:bootstrap` event.

### Option 2: Verify Hook is Active

```bash
neuro hooks list
```

Look for `self-improvement` in the output.
...
2. Start a new NEURO or Claude Code session
3. Send any prompt
...
1. **For NEURO**: `neuro hooks disable self-improvement`
2. **For Claude Code**: Comment out the hook in `settings.json` or delete the file
