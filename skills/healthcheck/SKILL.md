---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Neuro deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Neuro cron scheduling for periodic checks, or version status checks on a machine running Neuro (laptop, workstation, Pi, VPS).
---

# Neuro Host Hardening

## Overview

Assess and harden the host running Neuro, then align it to a user-defined risk tolerance without breaking access. Use Neuro security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Neuro changes the host firewall, SSH, or OS updates; it does not.
...
5. Neuro gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Neuro can access sensitive data.
...
### 2) Run Neuro security audits (read-only)

As part of the default read-only checks, run `neuro security audit --deep`. Only offer alternatives if the user requests them:

1. `neuro security audit` (faster, non-probing)
2. `neuro security audit --json` (structured output)

Offer to apply Neuro safe defaults (numbered):

1. `neuro security audit --fix`

Be explicit that `--fix` only tightens Neuro defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Neuro version/update status (read-only)

As part of the default read-only checks, run `neuro update status`.
...
- Credential hygiene notes (location of Neuro creds, prefer disk encryption)
...
After Neuro install or first hardening pass, run at least one baseline audit and version check:

- `neuro security audit`
- `neuro security audit --deep`
- `neuro update status`

Ongoing monitoring is recommended. Use the Neuro cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `neuro cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `neuro update status`
...
Before creating, `neuro cron list` and match on exact `name`. If found, `neuro cron edit <id> ...`.
If not found, `neuro cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `neuro update status` (preferred for source checkouts and channels)
2. `npm view neuro version` (published npm version)

## Neuro command accuracy

Use only supported commands and flags:

- `neuro security audit [--deep] [--fix] [--json]`
- `neuro status` / `neuro status --deep`
- `neuro health --json`
- `neuro update status`
- `neuro cron add|list|runs|run`

Do not invent CLI flags or imply Neuro enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Neuro compaction:
